...
def list_files():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, orig_name, stored_name, mime, size, uploaded_at FROM files ORDER BY id DESC"
    )
    rows = c.fetchall()
    conn.close()
    return rows
...
def get_file_record(file_id):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, orig_name, stored_name, mime, size, uploaded_at FROM files WHERE id=?",
        (file_id,),
    )
    row = c.fetchone()
    conn.close()
...
def upload():
    if request.method == "POST":
        ...
        stored_name = (
            base64.urlsafe_b64encode(os.urandom(9)).decode().rstrip("=") + ".bin"
        )
